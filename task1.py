...
class MyWidget(QMainWindow):
    def __init__(self):
        ...
        self.down.clicked.connect(self.uncor)
        self.left.clicked.connect(self.lef)
        self.right.clicked.connect(self.rig)
    ...
    def sc(self):
        if self.sender().text() == '+':
            self.scale1 = [self.scale1[0] * 0.5, self.scale1[1] * 0.5]
        else:
            self.scale1 = [self.scale1[0] / 0.5, self.scale1[1] / 0.5]
        self.push()
    ...
    def lef(self):
        if self.sender().text() == '<-':
            self.cords1 = [self.cords1[0], self.cords1[1]-self.scale1[0]*0.2]
            self.push()
